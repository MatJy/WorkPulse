...
export default function RealtimeSessionsBreaks({
  ...
  const [selectedSession, setSelectedSession] = useState<SessionsBreaks | null>(
    null
  );
  ...
  async function fetchSessionsAndBreaks() {
    ...
    if (!error && freshSessions) {
      ...
      const formattedData: SessionsBreaks[] = freshSessions.map((session) => ({
        session: {
          id: session.id,
            ...
            <div key={i} className='p-4'>
              ...
              <div className='bg-white shadow-md hover:shadow-lg transition rounded-xl border border-gray-200 p-6 max-w-xl  space-y-4'>
                ...
                <div className='grid grid-cols-2 gap-4 text-sm text-gray-600'>
                  <div>
                    <p className='font-medium text-gray-800'>Session length</p>
                    <p>{toHours(item.session.length)}</p>
                  </div>
                  <div>
                    <p className='font-medium text-gray-800'>Break every</p>
                    <p>{toHours(item.session.break_interval)}</p>
                  </div>
                  <div>
                    <p className='font-medium text-gray-800'>Time worked</p>
                    <p>
                      {item.session.minutes_worked != null
                        ? toHours(item.session.minutes_worked)
                        : 0}
                    </p>
                  </div>
                </div>
                ...
                <div className='md:flex justify-between items-center pt-4'>
                  <div className='flex justify-end space-x-2 pt-5'>
                    <button
                      ...
                      className='[background:linear-gradient(144deg,#af40ff,#5b42f3_50%,#00ddeb)] cursor-pointer text-white px-4 py-2 font-bold rounded-md hover:opacity-80 flex items-center justify-center'
                    >
                      {loadingSessionId === item.session.id ? (
                        <>
                          <svg
                            viewBox='0 0 24 24'
                            fill='none'
